...
Every webhook request includes an HMAC-SHA256 signature in the `x-paynode-signature` header. **You MUST verify this signature** to prevent spoofed webhook calls.
...
app.post('/api/paynode-webhook', (req, res) => {
  ...
  const signature = req.headers['x-paynode-signature'];
...
async def webhook(request: Request):
    ...
    signature = request.headers.get("x-paynode-signature", "")
...
| Header | Description |
| :--- | :--- |
| `x-paynode-signature` | `sha256={hmac_hex}` — HMAC-SHA256 of the JSON body |
